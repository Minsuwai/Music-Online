...
if (isset($_GET['edit'])) {
	$vinyl_id = $_GET['edit'];
	$result2= mysqli_query($conn, "SELECT * FROM vinyls WHERE v_id = $vinyl_id ");

	if (mysqli_num_rows($result2)>0) {
		while ($row2 = mysqli_fetch_assoc($result2)) {
			$albumcover = $row2['v_image'];
			?>
			...
			<div class="container-fluid aform">
				...
				if (isset($_POST['edit'])) {
					...
					if ($artist == '' || $album == '' || $year == '' || $genre == '' || $price == '') {
						...
						elseif ($uploadOk == 2) {
									mysqli_query($conn, "UPDATE vinyls SET v_artist = '$artist', v_album = '$album', v_releasedate = '$year', v_genre = '$genre', v_price = '$price' WHERE v_id = '$vinyl_id' "); // insert data to database
								...
								// if everthing is ok, try to upload file
									...
									if (file_exists($albumcover)) {
										...
										mysqli_query($conn,  "UPDATE vinyls SET v_artist = '$artist', v_album = '$album', v_releasedate = '$year', v_genre = '$genre', v_image = '$target_file', v_price = '$price' WHERE v_id = '$vinyl_id' "); // insert data to database
